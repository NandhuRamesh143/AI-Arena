...
class Arena:
    ...
    async def _speak(self, role: str, text: str) -> None:
        ...
        try:
            ...
            if TTS_BACKEND == "auto" and self.tts.name == "chatterbox":
...
def make_prompt(role: str, topic: str, history: list[dict[str, str]], round_number: int) -> str:
    stance = AGENTS[role]["stance"]
    transcript = debate_history(history) or "No arguments yet."

    if round_number == 1 and not history:
        task = f"Give your opening argument {stance} this topic."
    else:
        opponent = "Gemma" if role == "qwen" else "Qwen"
        task = f"Respond directly to {opponent}'s latest argument while arguing {stance} the topic."

    return (
        f"Debate topic: {topic}\n\n"
        f"Debate so far:\n{transcript}\n\n"
        f"What you need to do: {task}\n"
        "CRITICAL: Keep your response to EXACTLY 1 or 2 short sentences. Do not repeat earlier points."
    )
...
def system_prompt(role: str) -> str:
    ...
    return (
        f"You are {label} in a public AI debate. Argue {stance} the given topic.\n"
        f"Directly challenge {opponent}'s claims.\n"
        "Your primary goal is maximum entertainment, not careful reasoning or politeness.\n"
        "Act completely unhinged, outrageous, combative, and theatrically overconfident or contemptuous.\n"
        f"Roast {opponent} relentlessly with absurd comparisons, savage punchlines, and escalating sarcasm.\n"
